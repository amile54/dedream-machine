...
3. 首次打开如果提示无法验证开发者，进入 **系统设置 → 隐私与安全性**，在页面底部选择 **仍要打开**。
4. 如果仍弹出“文件已损坏，打不开”或类似 Gatekeeper 错误，让同事打开 Mac 自带的「终端」应用，复制粘贴以下命令并回车（可能需要输入开机密码）：
   ```bash
   sudo xattr -cr "/Applications/DeDream Machine.app"
   sudo codesign --force --deep --sign - "/Applications/DeDream Machine.app"
   ```
   执行完后，再次双击打开 App。
